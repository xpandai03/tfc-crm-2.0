...
import { buildIntakeRecord } from "./intake/build-intake";
...
export async function registerRoutes(
  ...
  app.post("/api/intake", async (req: any, res) => {
    try {
      ...
      // Every decision — what is rejected, what is stored, the timeline note —
      // is made by buildIntakeRecord (server/intake/build-intake.ts), which a
      // fixture can replay without a database. This route does the writes.
      const now = new Date().toISOString();
      const built = buildIntakeRecord(b, now);
      if (!built.ok) return res.status(built.status).json(built.body);
      const { isUploadedReferral, submissionSource, fields } = built;
      const intakeSource = fields.intakeSource;
